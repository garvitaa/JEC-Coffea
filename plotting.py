...
def fitProfile(output, x_var, y_var, norm_var, xlabel, ylabel, nbin):
    ...
    im = rax.scatter(bin_center, (offset_v_eta_data.values()[()]* (1/norm_data))/(offset_v_eta_mc.values()[()]* (1/norm_mc)),
    ...
    d_params= optimize.curve_fit(func, bin_center[20:104],
                                                   (offset_v_eta_data.values()[()]* (1/norm_data))[20:104],
                                                   p0=[2, 2, 2], full_output=True)
    m_params= optimize.curve_fit(func, bin_center[20:104],
                                                       (offset_v_eta_mc.values()[()]* (1/norm_mc))[20:104],
                                                       p0=[2, 2, 2], full_output=True)

    ax.plot(bin_center[20:104], func( bin_center[20:104], d_params[0][0], d_params[0][1], d_params[0][2]), 'k')
    ax.plot(bin_center[20:104], func( bin_center[20:104], m_params[0][0], m_params[0][1], m_params[0][2]), 'r')
...
def plotStack(output, x_var, y_var, norm_var, x_low, x_high, ylabel, flavor, ymax, ratioM):
    ...
    if flavor == 'all':
        ...
        ax.legend([(handles[i], handles[8+i][0]) for i in range(6)],
                  ['Photons','Neutral Hadrons', 'Unssoc. Charged Hadrons','Assoc. Charged Hadrons',
                   'EM Deposits', 'Hadron Deposits'], title='Markers: Data, Histograms: MC')
